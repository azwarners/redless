...
* `slow_local.yaml` - Recommended fork profile for `mini-slow`: tool-call mode,
  visible progress, a 10-minute Bash timeout, separate connection/read timeouts, no
...
llama.cpp. Set `model_name` and the endpoint in a small local settings file, then run
