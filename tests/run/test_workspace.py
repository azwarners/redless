...
def test_init_creates_a_safe_local_model_workspace(tmp_path):
    ...
    assert "model_name: local-model" in (path / ".redless" / "llama-local.yaml").read_text()
    assert "api_base: http://server:8080/v1" in (path / ".redless" / "llama-local.yaml").read_text()
    assert ".redless/" in (path / ".git" / "info" / "exclude").read_text()
    assert "MSWEA_CONFIGURED=true redless" in result.output
...
def test_clone_creates_a_configured_workspace(tmp_path):
    ...
    assert (destination / ".redless" / "llama-local.yaml").is_file()
